...
// tx.executeSql('CREATE TABLE IF NOT EXISTS TEMPVENTA2 (IdArt INTEGER,IdCli INTEGER)');
  ...
  tx.executeSql('INSERT INTO TEMPVENTA (IdArt,IdCli,Calibre,Empaque,Precio,Caja,Unidad,CajasCamion,CodMarca,DesArt,CodCaja, CodBotella) VALUES ('+v_save_art_id+','+v_cli_id+','+v_save_cal+','+v_save_emp+','+v_save_pre+','+v_save_caj+','+v_save_uni+','+v_save_caj_cam+','+v_save_cod_mar+',"'+v_save_des_art+'", (SELECT CodCaja FROM ARTICULO WHERE CodArt ='+v_save_art_id+') , (SELECT CodBotella FROM ARTICULO WHERE CodArt ='+v_save_art_id+'))');
